...
class PlayState(State):
    ...
    def __init__(self, engine):
        ...
        self.theme = get_environment_theme("city_day")
    ...
    def _init_pause_menu(self):
        ...
        self.pause_buttons = [
            MenuButton(pygame.Rect(cx, 260, btn_w, btn_h), "RESUME", "resume"),
            MenuButton(pygame.Rect(cx, 310, btn_w, btn_h), "RESTART", "restart"),
            MenuButton(pygame.Rect(cx, 360, btn_w, btn_h), "MAIN MENU", "menu", primary_color=COLOR_RED),
        ]

    def on_enter(self, track_data: Optional[TrackData] = None, **kwargs):
        self.is_paused = False
        ...
        # Spawner
        self.spawner.reset(start_y=0.0)
    ...
    def handle_events(self, events: list):
        ...
        # F3 Debug
        if input_mgr.is_action_just_pressed("debug"):
            debug.toggle()
        ...
        if debug.enabled:
            if input_mgr.is_just_pressed(pygame.K_g):
                ...
                self.spawner._spawn_traffic_car(self.player.y, difficulty=1.5)
            elif input_mgr.is_just_pressed(pygame.K_u):
                self.spawner._spawn_pickup(self.player.y)
    ...
    def update(self, dt: float):
        ...
        # Infinite Nitro Cheat
        if debug.infinite_nitro:
            self.player.nitro = 100.0

        # 1. Road Geometry Bounds at Player
        _, road_left, road_right = self.road_system.get_road_bounds(self.player.y)
        curve = self.road_system.get_curvature_at(self.player.y)
        ...
        # 3. Spawner Update
        self.spawner.update(scaled_dt, self.player.y, self.player.distance)

        # 4. Traffic AI Update
        for traffic in self.spawner.traffic_cars:
            traffic.update_ai(scaled_dt, self.spawner.traffic_cars, road_left=road_left)

        # 5. Pickups & Hazards Update
        for p in self.spawner.pickups:
            p.update(scaled_dt, self.player.x, self.player.y, magnet_active=(self.player.magnet_timer > 0))

        # 6. Particle System Update
        self.particle_system.update(scaled_dt)

        # 7. Collision Detection & Near-Miss Processing
        ...
        # 8. Dynamic Camera
        self.camera.update(scaled_dt, self.player.x, self.player.y, curve)
        ...
        self.engine.replay_mgr.record_frame({
            ...
            "traffic": [{"x": t.x, "y": t.y, "spr": t.sprite_name} for t in self.spawner.traffic_cars]
        })
    ...
    def render(self, surface: pygame.Surface):
        ...
        if self.player.is_nitro_active or self.player.speed > 420.0:
        ...
        if self.engine.debug.enabled:
            stats = {
                ...
                "pos_y": self.player.y,
                "speed": self.player.speed,
                "speed_kmh": (self.player.speed / 500.0) * 240.0,
                "fuel": self.player.fuel,
                "nitro": self.player.nitro,
                "health": self.player.health,
                "score": self.player.score,
                "track_name": self.road_system.track_name,
                "curvature": self.road_system.get_curvature_at(self.player.y),
            ...
            if self.engine.debug.show_hitboxes:
                ...
                for t in self.spawner.traffic_cars:
                    tsx, tsy = self.camera.world_to_screen(t.x, t.y)
                    thb = pygame.Rect(tsx - t.width // 2, tsy - t.height // 2, t.width, t.height)
                    self.engine.debug.draw_hitbox(surface, thb, (255, 0, 0))
        ...
        if self.is_paused:
            ...
            surface.blit(p_title, (VIRTUAL_WIDTH // 2 - p_title.get_width() // 2, 200))
