...
class GarageUI:
    ...
    def __init__(self):
        ...
        # Upgrades config: (key, name, base_cost)
        self.upgrade_defs = [
    ...
    def render(self, surface: pygame.Surface, career_profile: Dict[str, Any],
        ...
        surface.blit(title_surf, (VIRTUAL_WIDTH // 2 - title_surf.get_width() // 2, 14))

        # Credits Bar
        credits = career_profile.get("credits", 0)
        cr_surf = self.font_med.render(f"CREDITS: ${credits:,} CR", True, COLOR_GREEN)
        surface.blit(cr_surf, (VIRTUAL_WIDTH // 2 - cr_surf.get_width() // 2, 40))
    ...
    def _render_car_podium(self, surface: pygame.Surface, asset_pipeline, is_unlocked: bool):
        podium_rect = pygame.Rect(30, 68, VIRTUAL_WIDTH - 60, 150)
        ...
        cx, cy = podium_rect.centerx, podium_rect.centery - 10
        pulse = math.sin(pygame.time.get_ticks() * 0.005) * 5
        pygame.draw.ellipse(surface, (0, 180, 240, 100), (cx - 40 - pulse, cy + 20, 80 + pulse * 2, 24), width=2)

        # Draw vehicle sprite enlarged
        ...
        surface.blit(name_surf, (cx - name_surf.get_width() // 2, podium_rect.bottom - 36))

        if is_unlocked:
            stat_surf = self.font_mono.render("[UNLOCKED / ACTIVE]", True, COLOR_GREEN)
        else:
            stat_surf = self.font_mono.render(f"[LOCKED - UNLOCK: ${unlock_cost} CR]", True, COLOR_YELLOW)
        surface.blit(stat_surf, (cx - stat_surf.get_width() // 2, podium_rect.bottom - 18))
    ...
    def _render_upgrade_list(self, surface: pygame.Surface, profile: Dict[str, Any]):
        ...
        for i, (up_key, up_name, base_cost) in enumerate(self.upgrade_defs):
            ...
            card_rect = pygame.Rect(30, start_y + (i * (card_h + 8)), VIRTUAL_WIDTH - 60, card_h)
            ...
            if is_max:
                ...
                pr_surf = self.font_med.render(f"${cost} CR", True, pr_col)
            surface.blit(pr_surf, (card_rect.right - pr_surf.get_width() - 14, card_rect.centery - pr_surf.get_height() // 2))
